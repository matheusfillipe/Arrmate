...
import arrmate.config.instances as instances
import arrmate.config.service_config as service_config
...
def patch_sources(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        service_config,
        "load_saved_config",
        lambda: json.loads((tmp_path / "services.json").read_text()),
    )
    yield
